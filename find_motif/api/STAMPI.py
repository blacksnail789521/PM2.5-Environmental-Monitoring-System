...
def distanceprofile_mass(query, query_index, data_freq, data_length, data_sum2, data_sum, data_mean,  \
    ...
    distanceprofile[start : end] = [float("Inf")] * (end - start)

    return distanceprofile


def distanceprofile_ed(data, query_index, query_length, exclusion_zone):
    ...
    start = max(0, query_index - exclusion_zone)
    end = min(len(data) - query_length + 1, query_index + exclusion_zone + 1)
    distanceprofile[start : end] = [float("Inf")] * (end - start)
    
    return distanceprofile

    
def updatematrixprofile(distance_profile, matrix_profile, matrix_profile_index, data_length, \
                        query_length, query_index):
...
def matrixprofile(data, query_length, method):
    ...
    for i in range(len(data) - query_length + 1):
        ...
        #    pprint(i)
        if method == 0:
            distance_profile.append(distanceprofile_mass(data[i : i + query_length], i, data_freq, \
                                                    len(data), data_sum2, data_sum, \
                                                    data_mean, data_sigma2, data_sigma, exclusion_zone))
        elif method == 1:
            distance_profile.append(distanceprofile_ed(data, i, query_length, exclusion_zone))

    
    random_array = range(0, len(data) - query_length + 1)
...
def matrixprofileI(data, new_data_point, query_length, matrix_profile, matrix_profile_index):
    new_data = data[:]
    new_data.append(new_data_point)
    
    #new_query = new_data[len(new_data) - query_length : len(new_data)]
    ...
    new_distanceprofile = distanceprofile_mass(data[new_query_index : new_query_index + query_length], \
                                               new_query_index, data_freq, len(data), data_sum2, data_sum, \
                                               data_mean, data_sigma2, data_sigma, exclusion_zone)
    
    updatematrixprofile(new_distanceprofile, matrix_profile, matrix_profile_index, \
                        len(data), query_length, new_query_index)
    
    new_matrixprofile_point = min(new_distanceprofile)
    new_matrixprofileindex_point = new_distanceprofile.index(min(new_distanceprofile))
    
    new_matrix_profile = matrix_profile.append(new_matrixprofile_point)
    new_matrix_profile_index = matrix_profile_index.append(new_matrixprofileindex_point)
    
    return new_matrix_profile, new_matrix_profile_index
...
i = 0
pm25_csv = []    
with open('pm25.csv', 'rb') as csvfile:
