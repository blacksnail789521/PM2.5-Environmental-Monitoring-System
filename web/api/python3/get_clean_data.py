...
def daily__get_clean_data(device_id, date, sample_rate, start_or_end_or_neither):
    ...
    while slidewindow < data[0]["timestamp"]:
        ...
        if start_or_end_or_neither == "start":
    ...
    while final_data[len(final_data) - 1]["timestamp"] < final_data_end_timestamp:
        ...
        if start_or_end_or_neither == "end":
            ...
            final_data[len(final_data) - 1]["timestamp"] = slidewindow
            final_data[len(final_data) - 1]["pm25"] = None
            final_data[len(final_data) - 1]["temperature"] = None
            final_data[len(final_data) - 1]["humidity"] = None
...
def get_clean_data(device_id, start_date, end_date, sample_rate, days, hours):
    ...
    data = []
    start_or_end_or_neither = ""
    
    while current_date <= end_date:
        
        # print current_date (only Windows)
        if platform.system() == "Windows":
            print("processing " + str(current_date).split(" ")[0])
        
        # set start_or_end_or_neither (consider the head and tail of the data)
        
        if current_date == start_date - dt.timedelta(days = 1):
            start_or_end_or_neither = "start"
        elif current_date == end_date:
            start_or_end_or_neither = "end"
        else:
            start_or_end_or_neither = "neither"
        
        
        # get daily data
        
        daily_data = daily__get_clean_data(device_id, str(current_date).split(" ")[0], sample_rate, start_or_end_or_neither)
    ...
    for i in range(len(data)):
        if data[i]["pm25"] == None and data[i-1]["pm25"] != None:
            buf.append({})
            buf[b]["start"] = i
        elif data[i]["pm25"] != None and data[i-1]["pm25"] == None:
            buf[b]["end"] = i - 1
            b = b + 1
    ...
    for element in buf:
        ...
        before_none = data[element["start"] - 1]
        after_none = data[element["end"] + 1]
        
        for record_index in range(element["start"], element["end"] + 1):
            data[record_index]["pm25"] = int( before_none["pm25"] + (record_index - element["start"] + 1) * (after_none["pm25"] - before_none["pm25"]) /  (number_of_filled_value + 1) )
            data[record_index]["temperature"] = int( before_none["temperature"] + (record_index - element["start"] + 1) * (after_none["temperature"] - before_none["temperature"]) /  (number_of_filled_value + 1) )
            data[record_index]["humidity"] = int( before_none["humidity"] + (record_index - element["start"] + 1) * (after_none["humidity"] - before_none["humidity"]) /  (number_of_filled_value + 1) )
    
    
    # change UTC+0 to UTC+8
    
    for i in range(len(data)):
        data[i]["timestamp"] += dt.timedelta(hours = 8)
    
    
    # remove the data that time isn't in right date
    
    data = [record for record in data if (record["timestamp"].date() != start_date.date() - dt.timedelta(days = 1) and record["timestamp"].date() != end_date.date() + dt.timedelta(days = 1))]
    
    
    # remove the data that time isn't in days
    
    days_need_to_remove = [element for element in list(range(0, 7)) if element not in days]
    
    for element in days_need_to_remove:
        
        data = [record for record in data if not ( (record["timestamp"].isoweekday() if record["timestamp"].isoweekday() != 7 else 0) in days_need_to_remove )]
    ...
    for element in hours_need_to_remove:
        ...
        data = [record for record in data if not (hours_need_to_remove_start <= record["timestamp"].time() and record["timestamp"].time() <= hours_need_to_remove_end)]
